...
import { fetchJobs, resetJobs, incrementOffset, fetchJobDetail, setSelectedJobId } from "@/lib/redux/slices/jobSlice";
...
export default function VaultPage() {
   ...
   React.useEffect(() => {
     // No resetJobs() here - let Redux cache the list
     // fetchJobs has a 'condition' that will skip if data is fresh
     dispatch(fetchJobs({ 
       platform: selectedPlatform, 
       limit: 20, 
       offset: 0,
   ...
   const handleDraftCL = () => {
      ...
      // Simulate industrial-grade drafting pulse
      setTimeout(() => {
      ...
      <DashboardLayout>
         <div className="flex flex-col h-full bg-background/40 backdrop-blur-md rounded-[2.5rem] border-2 border-border/40 overflow-hidden shadow-2xl relative">
            ...
            <div className="flex-1 flex flex-col lg:flex-row min-h-0 relative border-t border-border/20">
               {/* Job List - Hidden on mobile if a job is selected */}
               <div className={cn(
               ...
               {/* Detailed Readout - Full screen on mobile when selected */}
               <div className={cn(
